...
echo "--- --compressed ---"
run_iters $ITERATIONS "ECHO_OK" "--compressed" "$LOCAL/echo"

echo "--- --write-out http_code ---"
for i in $(seq 1 $ITERATIONS); do
    TOTAL=$((TOTAL+1))
    out=$($CURLDBG -sS --write-out "CODE:%{http_code}" -o /dev/null "$LOCAL/echo" 2>/dev/null)
    if [ "$out" = "CODE:200" ]; then
        pass "--write-out http_code $LOCAL/echo"
    else
        fail "--write-out http_code $LOCAL/echo (got: $out)"
    fi
done
...
for i in $(seq 1 $ITERATIONS); do
    TOTAL=$((TOTAL+1))
    out=$($CURLDBG -sS --write-out "URL:%{url_effective}" -o /dev/null "$LOCAL/echo" 2>/dev/null)
    case "$out" in
        *"URL:$LOCAL/echo"*) pass "--write-out url_effective" ;;
        *) fail "--write-out url_effective (got: $out)" ;;
    esac
done

echo "--- --write-out num_redirects ---"
for i in $(seq 1 $ITERATIONS); do
    TOTAL=$((TOTAL+1))
    out=$($CURLDBG -sS --write-out "N:%{num_redirects}" -o /dev/null "$LOCAL/echo" 2>/dev/null)
    if [ "$out" = "N:0" ]; then
        pass "--write-out num_redirects"
    else
        fail "--write-out num_redirects (got: $out)"
    fi
done

echo "--- --data-urlencode =content ---"
run_iters $ITERATIONS "BODY:hello%20world" "--data-urlencode" "=hello world" "$LOCAL/echo"

echo "--- --data-urlencode plain ---"
run_iters $ITERATIONS "BODY:hello%20world" "--data-urlencode" "hello world" "$LOCAL/echo"

echo "--- --data-urlencode @file ---"
echo -n "file data" > /tmp/curldbg_urlenc_file.txt
run_iters $ITERATIONS "BODY:file%20data" "--data-urlencode" "@/tmp/curldbg_urlenc_file.txt" "$LOCAL/echo"
rm -f /tmp/curldbg_urlenc_file.txt

echo "--- --data-urlencode name=content ---"
run_iters $ITERATIONS "BODY:key=hello%20world" "--data-urlencode" "key=hello world" "$LOCAL/echo"

echo "--- --data-urlencode multiple ---"
run_iters $ITERATIONS "BODY:a=1&b=2" "--data-urlencode" "a=1" "--data-urlencode" "b=2" "$LOCAL/echo"

echo "--- --data-urlencode triggers POST ---"
run_iters $ITERATIONS "METHOD:POST" "--data-urlencode" "x=y" "$LOCAL/echo"

echo "--- --unix-socket error on nonexistent path ---"
for i in $(seq 1 $ITERATIONS); do
    run_check_exit 1 "--unix-socket" "/nonexistent/curldbg_test_sock" "$LOCAL/echo"
done

echo "--- Multi-URL (2 URLs) ---"
for i in $(seq 1 $ITERATIONS); do
    TOTAL=$((TOTAL+1))
    out=$($CURLDBG -sS "$LOCAL/echo" "$LOCAL/echo" 2>/dev/null)
    # Count occurrences (not lines) of ECHO_OK
    count=$(echo "$out" | grep -o "ECHO_OK" | wc -l || true)
    if [ "$count" -ge 2 ]; then
        pass "Multi-URL $LOCAL/echo $LOCAL/echo (got $count ECHO_OK)"
    else
        fail "Multi-URL $LOCAL/echo $LOCAL/echo (got $count ECHO_OK, out: $out)"
    fi
done

echo "--- --data-urlencode combined with -d ---"
run_iters $ITERATIONS "BODY:a=1&b=2" "-d" "a=1" "--data-urlencode" "b=2" "$LOCAL/echo"

echo "--- --cacert over HTTP (ignored, should succeed) ---"
run_iters $ITERATIONS "ECHO_OK" "--cacert" "/etc/ssl/certs/ca-certificates.crt" "$LOCAL/echo"

echo "--- --capath over HTTP (ignored, should succeed) ---"
run_iters $ITERATIONS "ECHO_OK" "--capath" "/etc/ssl/certs" "$LOCAL/echo"
